...
export async function GET(req: NextRequest) {
  ...
  const where = {
    isActive: true,
    ...(type && type !== 'all' ? { type: type as 'CONVENER' | 'FOREIGN' | 'FOREIGN_COUNSELLOR' | 'LOCAL_ADVISOR' | 'LOCAL_COUNSELLOR' | 'STAFF' | 'FULL_TIME' | 'PART_TIME' } : {}),
  }
...
const createSchema = z.object({
  ...
  type: z.enum(['CONVENER', 'FOREIGN', 'FOREIGN_COUNSELLOR', 'LOCAL_ADVISOR', 'LOCAL_COUNSELLOR', 'STAFF', 'FULL_TIME', 'PART_TIME']).default('FULL_TIME'),
