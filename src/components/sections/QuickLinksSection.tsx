...
export default function QuickLinksSection({ locale }: { locale: string }) {
  ...
  const links = [
    { href: '/admission', label: t('admission'), icon: FileText, color: 'bg-primary-600' },
    { href: '/news', label: t('calendar'), icon: Calendar, color: 'bg-primary-500' },
    { href: '/documents', label: t('documents'), icon: Download, color: 'bg-accent-500' },
    { href: '/contact', label: t('contact'), icon: Mail, color: 'bg-slate-600' },
  ]
