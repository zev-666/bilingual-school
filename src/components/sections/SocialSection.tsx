...
export default function SocialSection({ locale, facebookUrl, instagramUrl, youtubeUrl, lineUrl }: SocialSectionProps) {
  ...
  return (
    <section className="bg-[#E8F6EF] px-5 py-16 sm:px-8">
      <div className="mx-auto max-w-7xl">
        <Reveal>
          <h2 className="mb-8 text-center text-2xl font-bold text-[#2D241E]">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {items.map(({ key, url, icon: Icon, color }, index) => (
            <Reveal key={key} delay={index * 0.1}>
              <a href={url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 rounded-full border border-[#EFE9E1] bg-white px-6 py-3.5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_35px_rgba(195,78,36,.13)]">
